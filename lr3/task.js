function getUrlName() {
	const paramsString = new URLSearchParams(document.location.href);
		...
		for(var pair of searchParams.entries()) {
		   urlName = pair[0].replace("http://localhost:5000/users/", "");
...
var main = function (toDoObjects) {
	...
	tabs.push({
		...
		"content": function(callback) {
			$.getJSON("http://localhost:5000/clients/" + urlName, function (toDoObjects) {
				var $content = $("<ul>");
	...
	tabs.push({
		"name": "Теги",
		"content":function (callback) {
			$.get("http://localhost:5000/clients/" + urlName, function (toDoObjects) {	
